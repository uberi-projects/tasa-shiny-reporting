...
Fisheries-dependent data were collected opportunistically within the Turneffe Atoll Marine 
Reserve (TAMR) by TASA Conservation Officers during routine patrols and field operations. 
Officers conducted surveys with fishers encountered throughout the atoll, including those 
operating from skiffs, sailboats, or temporary fishing camps established within the reserve. 

Fishers typically conduct daily fishing trips lasting approximately six hours. To improve the 
accuracy of catch per unit effort (CPUE) estimates, Conservation Officers collected detailed, 
fisher-specific information on both time spent fishing and the composition of the catch. CPUE 
was calculated as the number of product units (individual conch meats, whole or headed 
lobster, or finfish) per fisher-hour (e.g., lobster/fisher-hr). 

For each fisher interviewed, officers recorded the date, fisher name, boat type, species caught, 
fishing gear used, and total fishing effort, measured to the nearest hour. Spatial data, including 
the location where fishers were encountered or where fishing activities were reported, were 
also documented to support spatial analyses of resource use across TAMR. 

All information was recorded using the SMART (Spatial Monitoring and Reporting Tool) mobile 
application, which standardizes data entry and integrates spatial positioning for improved data 
quality and management. 

Biological measurements were taken for individual catch items when encountered, although 
data completeness varied depending on the level of processing at the time of sampling. The 
following measurements were recorded: 

- Caribbean Spiny Lobster (Panulirus argus): total mass (g), tail mass (g), carapace length
(mm), sex, and reproductive indicators in females (presence of spermatophore or eggs). 
- Queen Conch (Aliger gigas): shell length (mm), total unprocessed tissue mass (g),
market-clean meat mass (g), and fillet mass (g). 
- Finfish: fork or tail length (cm) and total mass (g).

All mass measurements were recorded to the nearest 5–50 g based on the precision of the
measuring scales available at the time of sampling.
