...
if (as.character(params$lamp_1per_period_selection) != "None") {
  df_sites <- params$datafile$Sites |>
    filter(format(Date, "%b %Y") == params$lamp_1per_period_selection)
} else {
  df_sites <- params$datafile$Sites
}
...
df_urchin_crab <- if (study_urchin_crab_check) {
  params$datafile$Diadema_Crab %>%
    ...
    mutate(
      ...
      `Maguimithrax spinosissumus` = as.numeric(`Mithrax spinosissumus`)
    )
} else {
  data.frame(matrix(ncol = 0, nrow = 0))
}

# Redefine study taxa after merging ---------------------------
...
study_urchin_crab <- if (study_urchin_crab_check) "long-spined sea urchins (_Diadema antillarum_)"
study_urchin_crab2 <- if (study_urchin_crab_check) "Caribbean king crab (_Maguimithrax spinosissimus_)"
...
# Define study period ---------------------------
study_start <- as.Date(min(df_sites$Date, na.rm = TRUE))
study_end <- as.Date(max(df_sites$Date, na.rm = TRUE))
...
text_urchin_crab <- if (study_urchin_crab_check) {
  "Long-spined sea urchins (_Diadema antillarum_) are a species of Diadema urchins with long black spines that can reach 30cm from its body.
These urchins are essential in maintaining healthy coral reef ecosystems, through their behavior in grazing algae and increasing space for new corals to settle.
...
The Caribbean king crab (_Maguimithrax spinosissimus_) has a similar, though less studied, role in coral reef ecosystems as an algal herbivore.
...
text_lobster_methods <- if (study_lobster_check) {
  "Lobster specimens were assessed for carapace length (measured with a graduated tickle stick), sex, and egg presence (+/-)."
}
...
if (study_year >= 2024) {
  df_map <- df_sites |>
    ...
    distinct()
  if (length(unique(df_sites$`Site ID`)) < 50) {
  ...
  df_map <- df_sites |>
    ...
    distinct()
  if (length(unique(df_sites$`Site ID`)) < 50) {
    generate_map_historic(df_map, "Latitude", "Longitude", "Site ID")
  } else {
    generate_map_nolabels_historic(df_map, "Latitude", "Longitude", "Site ID")
  }
}
```
...
empty_counts <- c("0.0", "0", "NE", "N/E", NA, "", "N/A")
...
df_table1_sum_lobster <- if (study_lobster_check) {
  df_lobster %>%
    group_by(`Mgmt Zone`) %>%
    summarise(
      `Caribbean Spiny` = sum(Species == "Pa" | Species == "Panulirus argus" | Species == "Caribbean Spiny Lobster", na.rm = TRUE),
...
df_table1_sum_urchin_crab <- if (study_urchin_crab_check) {
  df_urchin_crab %>%
    ...
    summarise(
      `Diadema` = sum(
        `Adult Diadema antillarum`[!(`Adult Diadema antillarum` %in% empty_counts)],
        na.rm = TRUE
      ) +
        sum(
          `Juvenile Diadema antillarum`[!(`Juvenile Diadema antillarum` %in% empty_counts)],
          na.rm = TRUE
        ),
      `Maguimithrax` = sum(
        `Maguimithrax spinosissumus`[!(`Maguimithrax spinosissumus` %in% empty_counts)],
        na.rm = TRUE
      )
    ) %>%
    bind_rows(
...
df_table1 <- df_table1_sum_sites %>%
  ...
  mutate(Zone = case_when(
    Zone == "Special Management Zone" ~ "Special Mgmt Zone",
    TRUE ~ Zone
  ))
...
total_spiny <- if (study_lobster_check) {
  filter(df_table1, Zone == "Overall")$`Caribbean Spiny`
}
total_spotted <- if (study_lobster_check) {
  filter(df_table1, Zone == "Overall")$`Spotted Spiny`
}
total_urchin <- if (study_urchin_crab_check) {
  filter(df_table1, Zone == "Overall")$`Diadema`
}
total_crab <- if (study_urchin_crab_check) {
  filter(df_table1, Zone == "Overall")$`Maguimithrax`
}
...
df_table2_urchin_crab <- if (study_urchin_crab_check) {
  site_summaries <- df_urchin_crab %>%
    group_by(`Site ID`) %>%
    summarise(
      Diadema_Count =
        sum(`Adult Diadema antillarum`[!(`Adult Diadema antillarum` %in% empty_counts)], na.rm = TRUE) +
          sum(`Juvenile Diadema antillarum`[!(`Juvenile Diadema antillarum` %in% empty_counts)], na.rm = TRUE),
      Maguimithrax_Count =
        sum(`Maguimithrax spinosissumus`[!(`Maguimithrax spinosissumus` %in% empty_counts)], na.rm = TRUE),
      .groups = "drop"
    )
  zone_summary <- site_summaries %>%
    left_join(df_sites, by = "Site ID") %>%
    group_by(`Mgmt Zone`) %>%
    summarise(
      `Diadema / Site` = round(mean(Diadema_Count, na.rm = TRUE), 1),
      `Maguimithrax / Site` = round(mean(Maguimithrax_Count, na.rm = TRUE), 1),
      .groups = "drop"
    )
  overall_summary <- site_summaries %>%
    summarise(
      `Mgmt Zone` = "Overall",
      `Diadema / Site` = round(mean(Diadema_Count, na.rm = TRUE), 1),
      `Maguimithrax / Site` = round(mean(Maguimithrax_Count, na.rm = TRUE), 1)
    )
  bind_rows(zone_summary, overall_summary)
} else {
  tibble(`Mgmt Zone` = character())
}
...
overall_crab_av <- if (study_urchin_crab_check) {
  filter(df_table2, Zone == "Overall")$`Maguimithrax / Site`
}
...
tables1_2_text <- knitr::combine_words(na.omit(c(
  ...
  if (study_lobster_check) paste(total_spiny, "Caribbean Spiny lobster,", total_spotted, "Spotted Spiny lobster"),
  if (study_urchin_crab_check) paste(total_urchin, "_Diadema_"),
  if (study_urchin_crab_check) paste(total_crab, "_Maguimithrax_")
)))
tables1_2_text2 <- knitr::combine_words(na.omit(c(
  ...
  if (study_urchin_crab_check) paste(overall_crab_av, "_Maguimithrax_")
)))
...
finfish diversity (number of species), conch count, lobster count,  _Diadema_ count, and _Maguimithrax_ count. \
...
average  _Diadema_ count per site, and average _Maguimithrax_ count per site. \
...
if (study_conch_check) {
  ...
  df_table1co_overall <- df_conch %>%
    ...
    summarize(
      ...
      `Queen Conch / Ha` = round(`Queen Conch` / `Area (Ha)`, 1),
      `Milk Conch / Ha` = round(`Milk Conch` / `Area (Ha)`, 1)
    ) %>%
  ...
  conch_density_overall_queen <- filter(df_table1co, `Mgmt Zone` == "Overall")$`Queen Conch / Ha`
  conch_density_overall_milk <- filter(df_table1co, `Mgmt Zone` == "Overall")$`Milk Conch / Ha`
  conch_max_density <- max(filter(df_table1co, `Mgmt Zone` != "Overall")$`Queen Conch / Ha`)
  zone_conch_max_density <- filter(df_table1co, `Mgmt Zone` != "Overall")$`Mgmt Zone`[which.max(df_table1co$`Queen Conch / Ha`)]
}
```
...
if (study_conch_check) {
  ...
  df_figure1co <- df_table1co_zones %>%
    select(`Mgmt Zone`, `Queen Conch` = `Queen Conch / Ha`, `Milk Conch` = `Milk Conch / Ha`) %>%
...
if (study_lobster_check) {
  ...
  df_table1l_overall <- df_lobster %>%
    ...
    summarise(
      Sites = n_distinct(`Site ID`, na.rm = TRUE),
      `Ha Sampled` = n_distinct(paste0(`Site ID`, Transect), na.rm = TRUE) * 200 / 10000,
      `Caribbean Spiny` = sum(Species == "Pa" | Species == "Panulirus argus" | Species == "Caribbean Spiny Lobster"),
      `Caribbean Spiny / Ha` = round(`Caribbean Spiny` / `Ha Sampled`, 1),
      `Spotted Spiny` = sum(Species == "Pg" | Species == "Panulirus guttatus" | Species == "Spotted Spiny Lobster"),
  ...
  density_lobster_car <- df_table1l_overall$`Caribbean Spiny / Ha`
  max_density_lobster_car <- max(df_table1l_zones$`Caribbean Spiny / Ha`)
  zone_max_density_lobster_car <- df_table1l_zones$`Mgmt Zone`[which.max(df_table1l_zones$`Caribbean Spiny / Ha`)]
...
lobster_summary_text <- if (study_lobster_check) {
  paste(
    ...
    density_lobster_car, "individuals per hectare (ha).",
...
table1lcaption <- if (study_lobster_check) {
  paste(
    ...
    "at Turneffe Atoll Marine Reserve, represented by number of sites sampled,
    number of Caribbean spiny lobsters found, number of spotted spiny lobsters found, extrapolated density of Caribbean spiny lobsters found in individuals per hectare, and number of male and female Caribbean spiny lobsters found.",
    "\n\n"
  )
}
figure1lcaption <- if (study_lobster_check) {
  paste(
    "_Figure 1L._ Pie chart of lobster sex ratio (Panel A) and bar chart of lobster density
    by management zone (Panel B) in", study_year_month,
    " at Turneffe Atoll Marine Reserve.",
    "\n\n"
  )
}
...
if (study_lobster_check) {
  ...
  B <- ggplot(df_table1l_zones, aes(x = `Mgmt Zone`, y = `Caribbean Spiny / Ha`, fill = `Mgmt Zone`)) +
    geom_col(color = "black") +
    theme_custom +
    xlab("Management Zone") +
    labs(title = "B") +
...
`r if (study_urchin_crab_check){"### *Diadema* and *Maguimithrax*"}`
...
if (study_urchin_crab_check) {
  ...
  df_table1dm_overall <- df_urchin_crab %>%
    summarize(
      Sites = n_distinct(`Site ID`, na.rm = TRUE),
      `Adult Diadema` = sum(`Adult Diadema antillarum`, na.rm = TRUE),
      `Juvenile Diadema` = sum(`Juvenile Diadema antillarum`, na.rm = TRUE),
      `Maguimithrax` = sum(`Maguimithrax spinosissumus`, na.rm = TRUE)
    ) %>%
    mutate(`Mgmt Zone` = "Overall")
  ...
  total_mithrax <- df_table1dm_overall$`Maguimithrax`
}
```
...
# Create text about Diadema/Maguimithrax results ---------------------------
urchin_crab_summary_text <- if (study_urchin_crab_check) {
  paste(
    ...
    total_mithrax, "_Maguimithrax_ were found (Table 1DM; Figure 1DM).",
    "\n\n"
  )
}
# Create Diadema/Maguimithrax table/figure captions
table1dmcaption <- if (study_urchin_crab_check) {
  paste(
    "**Table 1DM.** _Diadema antillarum_ and _Maguimithrax spinosissumus_ counts in sampled management zones and overall in",
    study_year_month,
    "at Turneffe Atoll Marine Reserve.",
    "\n\n"
  )
...
figure1dmcaption <- if (study_urchin_crab_check) {
  paste(
    "_Figure 1DM_. Distribution of _Diadema antillarum_ and _Maguimithrax spinosissumus_ counts per site in",
    study_year_month,
    "at Turneffe Atoll Marine Reserve. Black Xs represent the mean for each group.",
    "\n\n"
  )
}
```
...
if (study_urchin_crab_check) {
  ...
  df_figure1dm <- df_urchin_crab %>%
    pivot_longer(
      cols = c(`Adult Diadema antillarum`, `Juvenile Diadema antillarum`, `Maguimithrax spinosissumus`),
