...
df$Month <- factor(df$Month, levels = unique(format(sort(unique(df$Date)), "%b %Y")), ordered = TRUE)
...
This report aims to summarize and visualize the SPAG dataset from `r study_timeframe` 
...
df_spec$Season <- as.character(df_spec$Season)
df_spec$Dive <- as.character(df_spec$Dive)
df_laser$Season <- as.character(df_laser$Season)
df_laser$Dive <- as.character(df_laser$Dive)
df_spec_base <- df_spec |>
  left_join(df, by = c("Season", "Dive")) |>
  filter(Site != "" & !is.na(Site))
df_spec_merged <- df_spec_base |>
...
total_sites <- length(unique(table1_separate$Site))
total_dives <- table1_overall$Dives
get_fish_or_zero <- function(df, sites) {
  val <- df |>
    filter(Site %in% sites) |>
    pull(Fish)
  if (length(val) == 0) 0 else sum(val)
}
total_mc <- format(get_fish_or_zero(table1_separate, c("Mauger Caye", "Maugre Caye")), big.mark = ",", scientific = FALSE)
total_cb <- format(get_fish_or_zero(table1_separate, c("Caye Bokel", "Bokel")), big.mark = ",", scientific = FALSE)
total_df <- format(get_fish_or_zero(table1_separate, c("Dog Flea", "Dog Flea Caye")), big.mark = ",", scientific = FALSE)
```

```{r table2_prep, echo = FALSE, fig.width = 7, fig.height = 7}
table2_months <- df_spec_base |>
  mutate(Species = `Species English Name`) |>
  filter(!is.na(Species) & Species != "") |>
  group_by(Species, Month) |>
  summarise(Count = sum(as.numeric(`Total Count`), na.rm = TRUE), .groups = "drop") |>
  pivot_wider(names_from = Month, values_from = Count, values_fill = 0) |>
  mutate(Total = rowSums(across(where(is.numeric))))
table2_sites <- df_spec_base |>
  mutate(Species = `Species English Name`) |>
  filter(!is.na(Species) & Species != "") |>
  group_by(Species) |>
  summarise(
    "Mauger Caye" = sum(as.numeric(`Total Count`)[Site %in% c("Mauger Caye", "Maugre Caye")], na.rm = TRUE),
    "Caye Bokel" = sum(as.numeric(`Total Count`)[Site %in% c("Caye Bokel", "Bokel Caye")], na.rm = TRUE),
    "Dog Flea" = sum(as.numeric(`Total Count`)[Site %in% c("Dog Flea Caye", "Dog Flea")], na.rm = TRUE),
    .groups = "drop"
  )
table2 <- table2_sites |>
  left_join(table2_months, by = "Species") |>
  mutate(Proportion = round(Total / total_fish_num, 3)) |>
  arrange(desc(Total)) |>
  slice_head(n = 10)
species_1 <- table2$Species[1]
count_1 <- format(table2$Total[1], scientific = FALSE)
prop_1 <- table2$Proportion[1] * 100
species_2 <- table2$Species[2]
count_2 <- format(table2$Total[2], scientific = FALSE)
prop_2 <- table2$Proportion[2] * 100
species_3 <- table2$Species[3]
count_3 <- format(table2$Total[3], scientific = FALSE)
...
top_species_fig2 <- df_spec_merged %>%
  mutate(`Total Count` = as.numeric(`Total Count`)) %>%
  group_by(`Species English Name`) %>%
  summarise(Total = sum(`Total Count`, na.rm = TRUE), .groups = "drop") %>%
  arrange(desc(Total)) %>%
  slice_head(n = 40) %>%
  pull(`Species English Name`)
df_fig2 <- df_spec_merged %>%
  mutate(`Total Count` = as.numeric(`Total Count`)) %>%
  filter(`Species English Name` %in% top_species_fig2) %>%
...
panels_present <- sort(unique(df_fig2$Panel))
caption_fig2 <- paste0(
  "Bar plot of observed fish species counts in ",
  paste(
    c(
      if ("A." %in% panels_present) "Mauger Caye (A)" else NULL,
      if ("B." %in% panels_present) "Caye Bokel (B)" else NULL,
      if ("C." %in% panels_present) "Dog Flea Caye (C)" else NULL
    ),
    collapse = ", "
  ),
  ", recorded during spawning aggregations at Turneffe ",
  "Atoll Marine Reserve, ", study_timeframe, "."
)
...
**Table 2.** Summary of SPAG monitoring by species, including how many of that fish were observed
 at Mauger Caye, Caye Bokel, and Dog Flea Caye, as well as during each month individually. 
Data were collected using SPAG data from Turneffe Atoll Marine Reserve, `r study_timeframe`.\

```{r table2, echo = FALSE, fig.width = 7, fig.height = 7}
table2_display <- as.data.frame(table2 |> select(-Proportion))
month_cols <- setdiff(names(table2_display), c("Species", "Mauger Caye", "Caye Bokel", "Dog Flea", "Total"))
flextable(table2_display) %>%
  bg(j = c("Mauger Caye", "Caye Bokel", "Dog Flea"), bg = "#ebeaea") %>%
  bg(j = month_cols, bg = "white") %>%
  bg(j = "Total", bg = "#ebeaea")
```
...
panels_present <- sort(unique(df_fig3$Panel))
caption_fig3 <- paste0(
  "Bar plot of observed fish family counts in ",
  paste(
    c(
      if ("A." %in% panels_present) "Mauger Caye (A)" else NULL,
      if ("B." %in% panels_present) "Caye Bokel (B)" else NULL,
...
size_class_cols <- c(
  "10-20 cm", "21-30 cm", "31-40 cm", "41-50 cm",
  "51-60 cm", "61-70 cm", "71-80 cm", "81-90 cm", "91-100 cm", "101-110 cm",
  "111-120 cm", "121-130 cm", "> 130 cm"
)
df_fig4 <- df_spec_base %>%
  select(`Species English Name`, `Total Count`, all_of(size_class_cols)) %>%
  mutate(across(all_of(size_class_cols), as.character)) %>%
  rowwise() %>%
  mutate(
    asterisk_count = sum(c_across(all_of(size_class_cols)) == "*", na.rm = TRUE),
    numeric_sum = sum(suppressWarnings(as.numeric(c_across(all_of(size_class_cols)))), na.rm = TRUE),
    remaining = as.numeric(`Total Count`) - numeric_sum,
    share = if_else(asterisk_count > 0 & remaining > 0,
      floor(remaining / asterisk_count), 0
    )
  ) %>%
  ungroup() %>%
  mutate(across(all_of(size_class_cols), ~ {
    is_asterisk <- .x == "*"
    numeric_val <- suppressWarnings(as.numeric(.x))
    if_else(is_asterisk, as.numeric(share), numeric_val)
  })) %>%
  select(-`Total Count`, -asterisk_count, -numeric_sum, -remaining, -share) %>%
  pivot_longer(
    cols = c(
      `10-20 cm`, `21-30 cm`, `31-40 cm`, `41-50 cm`,
      `51-60 cm`, `61-70 cm`, `71-80 cm`, `81-90 cm`, `91-100 cm`, `101-110 cm`,
...
df_visuals <- df_spec_base %>%
  select(`Total Count`, all_of(size_class_cols)) %>%
  mutate(across(all_of(size_class_cols), as.character)) %>%
  rowwise() %>%
  mutate(
    has_size_data = any(!is.na(c_across(all_of(size_class_cols))) &
      c_across(all_of(size_class_cols)) != "", na.rm = TRUE),
    numeric_sum = sum(suppressWarnings(as.numeric(c_across(all_of(size_class_cols)))), na.rm = TRUE),
    has_asterisk = any(c_across(all_of(size_class_cols)) == "*", na.rm = TRUE),
    visual_count = if_else(has_size_data,
      if_else(has_asterisk, as.numeric(`Total Count`), numeric_sum),
      0
    )
  ) %>%
  ungroup()
total_visuals <- sum(df_visuals$visual_count, na.rm = TRUE)
...
plot_or_message <- function(data, site_name, palette, title) {
  ...
  if (!has_data) {
    ggplot() +
      ...
      theme_void() +
      theme(plot.title = element_text(size = 18, margin = margin(0, 0, 5, 0))) +
      labs(title = title)
  } else {
