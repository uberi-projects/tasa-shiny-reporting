...
most_family <- families$`Family.x`[which.max(families$`Family.x`)]
```
...
nassau_size <- round(mean(filter(df_fig5, Species == "Nassau Grouper")$`Fish Measurement (cm)`), 2)
```
...
The average laser-measured size of Nassau Grouper was `r nassau_size`cm (Figure 5). 
...
Atoll Marine Reserve, `r study_timeframe`. Only the four fish species with the 
