...
} else if (timeframe == "Seasonal" && as.character(params$fisher_multiper1_lobster_season_selection) != "None") {
  season_years <- unlist(strsplit(params$fisher_multiper1_lobster_season_selection, "-"))
...
} else if (timeframe == "Seasonal" && as.character(params$fisher_multiper2_lobster_season_selection) != "None") {
  season_years <- unlist(strsplit(params$fisher_multiper2_lobster_season_selection, "-"))
  start_date <- as.Date(paste0(season_years[1], "-07-01"))
  end_date <- as.Date(paste0(season_years[2], "-02-28"))
  df_2 <- df_2 |>
    filter(`Waypoint Date` >= start_date, `Waypoint Date` <= end_date)
...
if (!is.null(params$datafile3)) {
  ...
  } else if (timeframe == "Seasonal" && as.character(params$fisher_multiper3_lobster_season_selection) != "None") {
    season_years <- unlist(strsplit(params$fisher_multiper3_lobster_season_selection, "-"))
    start_date <- as.Date(paste0(season_years[1], "-07-01"))
    end_date <- as.Date(paste0(season_years[2], "-02-28"))
    df_3 <- df_3 |>
      filter(`Waypoint Date` >= start_date, `Waypoint Date` <= end_date)
...
if (!is.null(params$datafile4)) {
  ...
  } else if (timeframe == "Seasonal" && as.character(params$fisher_multiper4_lobster_season_selection) != "None") {
    season_years <- unlist(strsplit(params$fisher_multiper4_lobster_season_selection, "-"))
    start_date <- as.Date(paste0(season_years[1], "-07-01"))
    end_date <- as.Date(paste0(season_years[2], "-02-28"))
    df_4 <- df_4 |>
      filter(`Waypoint Date` >= start_date, `Waypoint Date` <= end_date)
...
Data on carapace length (Inches) and unheaded weight (oz) were entered by fishermen using SMART 
...
**Table 1.** Summary of fishing activity by time period, including total fishers, events, and lobster caught, 
...
Data were collected using SMART lobster data from fishers at Turneffe Atoll Marine Reserve, `r study_timeframe`.\
...
The CPUE of lobster in pounds per hour had a 
...
if (timeframe == "Annual") {
  ...
  B <- ggplot(df_fig2_A, aes(x = Year, y = Hours, group = 1)) +
    ...
    ylab("Hours Fished") +
    labs(title = "B")
  C <- ggplot(df_fig2_A, aes(x = Year, y = Lobster, group = 1)) +
    geom_line(size = 0.5) +
    geom_point(size = 2.5) +
    expand_limits(y = 0) +
    theme_custom +
...
if (timeframe == "Seasonal") {
  ...
  B <- ggplot(df_fig2_A, aes(x = Season, y = Hours, group = 1)) +
    ...
    ylab("Hours Fished") +
    labs(title = "B")
  C <- ggplot(df_fig2_A, aes(x = Season, y = Lobster, group = 1)) +
    geom_point(size = 2.5) +
    expand_limits(y = 0) +
    theme_custom +
    ylab("Number Lobsters") +
...
if (timeframe == "Annual") {
  ...
  total_male <- filter(df_table3, Year == "Overall")$`Male (%)`
  total_female <- filter(df_table3, Year == "Overall")$`Female (%)`
  total_carapace <- filter(df_table3, Year == "Overall")$`Mean Carapace Length (In)`
  total_weight <- filter(df_table3, Year == "Overall")$`Mean Weight (oz)`
  max_carapace <- max(filter(df_table3, Year != "Overall")$`Mean Carapace Length (In)`, na.rm = TRUE)
  max_carapace_year <- df_table3$Year[which.max(filter(df_table3, Year != "Overall")$`Mean Carapace Length (In)`)]
  ...
  max_female <- max(filter(df_table3, Year != "Overall")$`Female (%)`, na.rm = TRUE)
  max_female_year <- df_table3$Year[which.max(filter(df_table3, Year != "Overall")$`Female (%)`)]
  max_male <- max(filter(df_table3, Year != "Overall")$`Male (%)`, na.rm = TRUE)
  max_male_year <- df_table3$Year[which.max(filter(df_table3, Year != "Overall")$`Male (%)`)]
}
...
if (timeframe == "Seasonal") {
  ...
  # Prepare dynamic variables for Table 3 ---------------------------
  total_male <- filter(df_table3, Season == "Overall")$`Male (%)`
  total_female <- filter(df_table3, Season == "Overall")$`Female (%)`
  total_carapace <- filter(df_table3, Season == "Overall")$`Mean Carapace Length (In)`
  total_weight <- filter(df_table3, Season == "Overall")$`Mean Weight (oz)`
  max_carapace <- max(filter(df_table3, Season != "Overall")$`Mean Carapace Length (In)`, na.rm = TRUE)
  max_carapace_year <- df_table3$Season[which.max(filter(df_table3, Season != "Overall")$`Mean Carapace Length (In)`)]
  ...
  max_female <- max(filter(df_table3, Season != "Overall")$`Female (%)`, na.rm = TRUE)
  max_female_year <- df_table3$Season[which.max(filter(df_table3, Season != "Overall")$`Female (%)`)]
  max_male <- max(filter(df_table3, Season != "Overall")$`Male (%)`, na.rm = TRUE)
  max_male_year <- df_table3$Season[which.max(filter(df_table3, Season != "Overall")$`Male (%)`)]
}
```


Average carapace length was `r total_carapace`in, greatest in `r max_carapace_year` with `r max_carapace`in. 
Average headed weight was `r total_weight`oz, greatest in `r max_weight_year` with `r max_weight`oz. 
Overall, there was `r total_female`% female and `r total_male`% male lobsters identified, 
with the highest percent female of `r max_female` found in `r max_female_year`, 
and the highest percent male of `r max_male` found in `r max_male_year` (Table 3; Figure 3; Figure 4).\


**Table 3.** Summary of mean and median carapace length (In) and headed weight (oz), 
...
if (timeframe == "Annual") {
  ...
  C <- ggplot(df_fig3_unsummarized, aes(x = weight_for_headed, fill = `Type of Fishing Gears`)) +
    ...
    labs(title = "C") +
    theme(legend.position = "none")
  D <- ggplot(df_fig3_carapace, aes(x = Year, y = `Mean Carapace Length (In)`)) +
    geom_line(size = 0.5) +
    geom_point(size = 2.5) +
    theme_custom +
    ...
    labs(title = "D") +
    theme(legend.position = "none")
  E <- ggplot(df_fig3_weight, aes(x = Year, y = `Mean Weight (oz)`)) +
    geom_line(size = 0.5) +
    geom_point(size = 2.5) +
    theme_custom +
    ylab("Weight (oz)") +
...
if (timeframe == "Seasonal") {
  ...
  C <- ggplot(df_fig3_unsummarized, aes(x = weight_for_headed, fill = `Type of Fishing Gears`)) +
    ...
    labs(title = "C") +
    theme(legend.position = "none")
  D <- ggplot(df_fig3_carapace, aes(x = Season, y = `Mean Carapace Length (In)`)) +
    geom_line(size = 0.5) +
    geom_point(size = 2.5) +
    theme_custom +
    ylab("Mean Carapace Length (In)") +
    ...
    labs(title = "D") +
    theme(legend.position = "none")
  E <- ggplot(df_fig3_weight, aes(x = Season, y = `Mean Weight (oz)`)) +
    geom_line(size = 0.5) +
    geom_point(size = 2.5) +
    theme_custom +
    ylab("Weight (oz)") +
...
histogram of carapace length (Inches) by gear type (B), histogram of headed weight (oz) by gear type (C), 
line plot of mean carapace length (In) over time (D), and line plot of headed mean weight (oz) over time (E), 
...
if (timeframe == "Annual") {
  # Create annual dfs for Figure 4 ---------------------------
  df_fig4 <- df %>%
    mutate(
      Year = format(df$`Waypoint Date`, "%Y")
    ) %>%
  ...
  # Create Figure 4 ---------------------------
  figure4 <- ggplot(df_fig4, aes(x = Year, fill = Sex, y = Percent)) +
...
if (timeframe == "Seasonal") {
  # Create seasonal dfs for Figure 4 ---------------------------
  df_fig4 <- df %>%
    group_by(Season) %>%
    summarize(
      Female = round(100 * sum(Sex == "Female", na.rm = TRUE) / n(), 1),
  ...
  # Create Figure 4 ---------------------------
  figure4 <- ggplot(df_fig4, aes(x = Season, fill = Sex, y = Percent)) +
    geom_col(color = "black") +
    scale_fill_manual(values = palette[2:3]) +
    theme_custom +
    ylab("Percent")
