...
if (as.character(params$fisher_multiper1_year_selection) != "None") {
  df_1 <- df_1 |>
...
if (as.character(params$fisher_multiper2_year_selection) != "None") {
  df_2 <- df_2 |>
...
if (!is.null(params$datafile3)) {
  ...
  if (as.character(params$fisher_multiper3_year_selection) != "None") {
    df_3 <- df_3 |>
...
if (!is.null(params$datafile4)) {
  ...
  if (as.character(params$fisher_multiper4_year_selection) != "None") {
    df_4 <- df_4 |>
...
**Table 1.** Summary of fishing activity by year, including total fishers, events, and finfish caught, 
...
B <- ggplot(df_fig2_A, aes(x = Year, y = Hours, group = 1)) +
  ...
  ylab("Hours Fished") +
  labs(title = "B")
C <- ggplot(df_fig2_A, aes(x = Year, y = Finfish, group = 1)) +
  geom_line(size = 0.5) +
  geom_point(size = 2.5) +
  expand_limits(y = 0) +
  theme_custom +
...
total_weight_gutted <- filter(df_table3, Year == "Overall")$`Mean Weight Gutted (lbs)`
max_weight_gutted <- max(filter(df_table3, Year != "Overall")$`Mean Weight Gutted (lbs)`, na.rm = TRUE)
max_weight_gutted_month <- df_table3$Year[which.max(filter(df_table3, Year != "Overall")$`Mean Weight Gutted (lbs)`)]
total_weight_whole <- filter(df_table3, Year == "Overall")$`Mean Weight Whole (lbs)`
max_weight_whole <- max(filter(df_table3, Year != "Overall")$`Mean Weight Whole (lbs)`, na.rm = TRUE)
max_weight_whole_month <- df_table3$Year[which.max(filter(df_table3, Year != "Overall")$`Mean Weight Whole (lbs)`)]
total_total_length <- filter(df_table3, Year == "Overall")$`Mean Length (In)`
max_total_total_length <- max(filter(df_table3, Year != "Overall")$`Mean Length (In)`, na.rm = TRUE)
max_total_total_length_month <- df_table3$Year[which.max(filter(df_table3, Year != "Overall")$`Mean Length (In)`)]
total_fork_length <- filter(df_table3, Year == "Overall")$`Mean Fork Length (In)`
max_total_fork_length <- max(filter(df_table3, Year != "Overall")$`Mean Fork Length (In)`, na.rm = TRUE)
max_total_fork_length_month <- df_table3$Year[which.max(filter(df_table3, Year != "Overall")$`Mean Fork Length (In)`)]
```
...
C <- ggplot() +
  geom_line(
    ...
    aes(x = Year, y = `Mean Length (In)`, linetype = "Total Length"),
    size = 0.5
  ) +
  geom_point(
    data = df_fig3_total_length,
    aes(x = Year, y = `Mean Length (In)`, shape = "Total Length"),
    size = 2.5
  ) +
  geom_line(
    data = df_fig3_fork_length,
    aes(x = Year, y = `Mean Fork Length (In)`, linetype = "Fork Length"),
    size = 0.5
  ) +
  geom_point(
    data = df_fig3_fork_length,
    aes(x = Year, y = `Mean Fork Length (In)`, shape = "Fork Length"),
    size = 2.5
  ) +
  theme_custom +
...
D <- ggplot(df_fig3_gutted_weight, aes(x = Year, y = `Mean Weight Gutted (lbs)`)) +
  geom_line(size = 0.5) +
  geom_point(size = 2.5) +
  theme_custom +
  ylab("Gutted Weight (lb)") +
  labs(title = "D")
E <- ggplot(df_fig3_whole_weight, aes(x = Year, y = `Mean Weight Whole (lbs)`)) +
  geom_line(size = 0.5) +
  geom_point(size = 2.5) +
  theme_custom +
...
unique_years <- unique(arrange(df_figure4, Year)$Year)
common_fish_text <- c()
for (year in unique_years) {
  common_fish_text <- c(
    common_fish_text, paste0(
      ...
      year,
      " was ",
      arrange(filter(df_figure4, Year == year), desc(Count))$`Fish Species`[1]
    )
  )
}
...
Across years, `r common_fish_text` (Figure 4).\
...
*Figure 4.* Column plot of commonly caught fish species by year in which they were caught, 
collected using SMART finfish data from fishers at Turneffe 
Atoll Marine Reserve, `r study_timeframe`.\
