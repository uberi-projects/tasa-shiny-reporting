...
if (!is.null(params$datafile4)) {
  study4_start <- min(params$datafile4$Survey_Data$Date, na.rm = TRUE)
  study4_period <- paste0(study4_start, " to ", max(params$datafile4$Survey_Data$Date, na.rm = TRUE))
  study4_year <- format(study4_start, "%Y")
}
...
if (!is.null(params$datafile4)) {
  ...
  study_years <- c(study_years, study4_year)
}
...
if (!is.null(params$datafile4)) {
  df_surveys4 <- update_site_id(params$datafile4$Survey_Data)
  df_sites4 <- update_site_id(params$datafile4$Sites)
}
correct_site_id <- function(df, site_col = "Site ID") {
  df %>%
    mutate(
      !!site_col := str_replace(
        !!sym(site_col),
        "^([A-Z]+)(\\d{1})$",
        "\\10\\2"
      )
...
df_surveys1 <- correct_site_id(params$datafile1$Survey_Data)
df_sites1 <- correct_site_id(params$datafile1$Sites)
df_surveys2 <- correct_site_id(params$datafile2$Survey_Data)
df_sites2 <- correct_site_id(params$datafile2$Sites)
if (!is.null(params$datafile3)) {
  df_surveys3 <- correct_site_id(params$datafile3$Survey_Data)
  df_sites3 <- correct_site_id(params$datafile3$Sites)
}
if (!is.null(params$datafile4)) {
  df_surveys4 <- correct_site_id(params$datafile4$Survey_Data)
  df_sites4 <- correct_site_id(params$datafile4$Sites)
}
...
if (!is.null(params$datafile4)) {
  df_merged4 <- df_surveys4 |>
    left_join(df_sites4, by = "Site ID") |>
    filter(Section != "NW") |>
    mutate(`Conch Count` = as.integer(`Conch Count`), Year = study4_year)
  merged_list <- append(merged_list, list(df_merged4))
}
...
if (!is.null(params$datafile4)) {
  df_list <- append(df_list, list(df_merged4))
}
...
if (!is.null(params$datafile4)) {
  number_sites4 <- length(unique(df_merged4$`Site ID`))
  all_sites <- c(all_sites, df_merged4$`Site ID`)
  site_sets <- append(site_sets, list(df_merged4$`Site ID`))
}
...
The Long-term Atoll Monitoring Program (LAMP) at TAMR, adapted from the original LAMP protocol for Glover’s Reef (Acosta 2004), is designed to monitor key fished taxa at the atoll over space and time, including finfish, queen conch (_Aliger gigas_) and milk conch (_Strombus costatus_), Caribbean spiny lobster (_Panulirus argus_) and spotted spiny lobster (_Panulirus guttatus_), black sea urchins (_Diadema antillarum_), and Caribbean king crab (_Maguimithrax spinosissumus_). 
...
Please note that management zones are here presented using the new polygons established in 2022, as opposed to the historic ones.\
...
if (!is.null(params$datafile4)) {
  datafile4_table1_results_text <- paste0(
    ". In ", study4_year,
    ", ", df_table1_list$NE_sites4, " sites were sampled in the Northeast, ",
    df_table1_list$NO_sites4, " in the North, ",
    df_table1_list$SE_sites4, " in the Southeast, and ",
    df_table1_list$SW_sites4, " in the Southwest, with ",
    df_table1_list$total_conch4, " counted conch"
  )
} else {
  datafile3_table1_results_text <- ""
  datafile4_table1_results_text <- ""
}
```
...
`r datafile3_table1_results_text``r datafile4_table1_results_text` (Table 1).\
...
if (!is.null(params$datafile4)) {
  datafile4_skew_kurtosis_text <- paste0(
    ". The distribution of Queen Conch counts in ",
    study4_year, " has a ", skew_kurtosis_list$skew_intensity[[study4_year]], " ",
    skew_kurtosis_list$skew_direction[[study4_year]], " skew of ",
    skew_kurtosis_list$overall_skewness[[study4_year]], " and a ",
    skew_kurtosis_list$kurtosis_classification[[study4_year]], " kurtosis of ",
    skew_kurtosis_list$overall_kurtosis[[study4_year]], ", ",
    "suggesting that Queen Conch counts are typically ",
    skew_kurtosis_list$skew_presence_interpretation[[study4_year]], " on transects, ",
    "with ", skew_kurtosis_list$kurtosis_presence_interpretation[[study4_year]]
  )
} else {
  datafile4_skew_kurtosis_text <- ""
}
...
`r datafile3_skew_kurtosis_text``r datafile4_skew_kurtosis_text` (Table 2; Figure 2). 
...
if (!is.null(params$datafile3)) {
  ...
  if (!is.null(params$datafile4)) {
    D <- ggplot(
      filter(summary_site, Year == study4_year),
      aes(x = Section, y = Conch_Count_Site, fill = Section)
    ) +
      geom_boxplot(fill = "white") +
      geom_point(
        data = filter(mean_per_site, Year == study4_year),
        aes(x = Section, y = Mean_Conch_Per_Site),
        size = 5, shape = 2, stroke = 1
      ) +
      theme_custom +
      ...
      ylab("") +
      guides(fill = "none") +
      labs(title = "D", subtitle = study4_year) +
      scale_y_continuous(limits = c(0, max_y))
...
if (!is.null(params$datafile3) & !is.null(params$datafile4)) {
  datafile3_fig2_caption <- paste0(" and ", study3_year, " (C) and ", study4_year, " (D)")
} else if (!is.null(params$datafile3)) {
...
# Conditionally assemble Figure 2 ---------------------------
if (!is.null(params$datafile3)) {
  C <- ggplot(filter(df_fig3, Year == study3_year), aes(x = `Shell Length (cm)`, fill = Lipped)) +
    geom_histogram(color = "black", position = "identity", alpha = 0.5) +
    theme_custom +
    labs(title = "C", subtitle = study3_year) +
    scale_fill_manual(values = c(palette[3], palette[4])) +
    ...
    scale_x_continuous(limits = c(0, 25), breaks = seq(0, 25, 5))
  if (!is.null(params$datafile4)) {
    D <- ggplot(filter(df_fig3, Year == study4_year), aes(x = `Shell Length (cm)`, fill = Lipped)) +
      geom_histogram(color = "black", position = "identity", alpha = 0.5) +
      theme_custom +
      labs(title = "D", subtitle = study4_year) +
      scale_fill_manual(values = c(palette[3], palette[4])) +
...
if (!is.null(params$datafile3) & !is.null(params$datafile4)) {
  datafile3_fig3_caption <- paste0(" and ", study3_year, " (C) and ", study4_year, " (D)")
} else if (!is.null(params$datafile3)) {
  datafile3_fig3_caption <- paste0(" and ", study3_year, " (C)")
} else {
  datafile3_fig3_caption <- ""
}
```
...
for data from `r study1_year` (A), `r study2_year` (B)`r datafile3_fig3_caption` at Turneffe Atoll Marine Reserve. 
