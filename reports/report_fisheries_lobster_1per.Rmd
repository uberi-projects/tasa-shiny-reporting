...
} else if (timeframe == "Seasonal" && as.character(params$fisheries_1per_lobster_season_selection) != "None") {
  season_years <- unlist(strsplit(params$fisheries_1per_lobster_season_selection, "-"))
...
total_male <- filter(df_table2, Month == "Overall")$`Male (%)`
total_female <- filter(df_table2, Month == "Overall")$`Female (%)`
total_carapace <- filter(df_table2, Month == "Overall")$`Mean Carapace Length (mm)`
total_weight <- filter(df_table2, Month == "Overall")$`Mean Weight (g)`
max_carapace <- max(filter(df_table2, Month != "Overall")$`Mean Carapace Length (mm)`, na.rm = TRUE)
max_carapace_month <- df_table2$Month[which.max(filter(df_table2, Month != "Overall")$`Mean Carapace Length (mm)`)]
max_weight <- max(filter(df_table2, Month != "Overall")$`Mean Weight (g)`, na.rm = TRUE)
max_weight_month <- df_table2$Month[which.max(filter(df_table2, Month != "Overall")$`Mean Weight (g)`)]
max_female <- max(filter(df_table2, Month != "Overall")$`Female (%)`, na.rm = TRUE)
max_female_month <- df_table2$Month[which.max(filter(df_table2, Month != "Overall")$`Female (%)`)]
max_male <- max(filter(df_table2, Month != "Overall")$`Male (%)`, na.rm = TRUE)
max_male_month <- df_table2$Month[which.max(filter(df_table2, Month != "Overall")$`Male (%)`)]
```


Average carapace length was `r total_carapace`in, greatest in `r max_carapace_month` with `r max_carapace`in. 
Average headed weight was `r total_weight`g, greatest in `r max_weight_month` with `r max_weight`g. 
...
and the highest percent male of `r max_male` found in `r max_male_month` (Table 2; Figure 2; Figure 3).\
