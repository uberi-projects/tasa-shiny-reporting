...
df_table3 <- df %>%
  ...
  group_by(Month, Month_sort, `Fish Species`) %>%
  summarize(Count = n()) %>%
  filter(`Fish Species` %in% df_table3_overall$`Fish Species`) %>%
  pivot_wider(names_from = `Fish Species`, values_from = Count, values_fill = 0) %>%
  arrange(Month_sort) %>%
...
months <- unique(filter(df_table3, Month != "Overall")$Month)
months_top <- c()
for (month in months) {
  df_month <- filter(df_table3, Month == month)
  species_max <- names(df_month)[which.max(df_month[1, -1])]
  months_top <- c(months_top, paste0(species_max, " in ", month))
}
months_top_words <- combine_words(months_top)
```
...
The most common species observed in each month include `r months_top_words`.
(Figure 4; Table 3).
...
**Table 3.** Count of observed finfish species in the surveyed months, including only the top five most 
...
df_table4_notoverall <- filter(df_table4, Month != "Overall")

# Prepare dynamic variables for Table 4 ---------------------------
max_skiff <- max(df_table4_notoverall$Skiff, na.rm = TRUE)
max_skiff_month <- df_table4_notoverall$Month[which.max(df_table4_notoverall$Skiff)]
max_canoe <- max(df_table4_notoverall$`Canoe/Sailboat`, na.rm = TRUE)
max_canoe_month <- df_table4_notoverall$Month[which.max(df_table4_notoverall$`Canoe/Sailboat`)]
```
...
wheres sailboats/canoes were used the most in `r max_canoe_month` with `r max_canoe` instances (Table 4; Figure 5).\


**Table 4.** Count of vessel type counts for each month, where one count corresponds to a vessel submitting data for a single mission. 
Data were collected using Fisheries catch finfish data from Turneffe Atoll Marine Reserve, `r study_timeframe`.\
...
df_fig5 <- df %>%
  ...
  complete(`Vessel Type`, Month_sort, fill = list(Vessels = 0))

# Create Figure 5 ---------------------------
figure5 <- ggplot(df_fig5, aes(x = Month_sort, color = `Vessel Type`, y = Vessels)) +
  geom_line() +
...
*Figure 5.* Line and scatter plot of vessel type counts for each month, where one count corresponds to a vessel submitting data for a single mission. 
Data were collected using Fisheries catch finfish data from Turneffe Atoll Marine Reserve, `r study_timeframe`.\
