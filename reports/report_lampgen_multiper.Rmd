...
if (study_conch_check) {
  ...
  finfish_checks <- c(
    study1_year_month = study_finfish_check1,
    study2_year_month = study_finfish_check2,
    study3_year_month = study_finfish_check3,
    study4_year_month = study_finfish_check4
  )
  finfish_panels <- names(finfish_checks[finfish_checks])

  # Create panels for Figure 2Co ---------------------------
  if (length(finfish_panels) == 1) {
    ...
    A <- ggplot(filter(df_figure2co, Survey == get(finfish_panels[1])), aes(x = `Shell Length (cm)`, fill = Lipped)) +
      geom_histogram(color = "black", position = "identity", alpha = 0.5) +
      theme_custom +
      labs(title = "A", subtitle = get(finfish_panels[1])) +
      scale_fill_manual(values = c(palette[3], palette[4])) +
      geom_vline(xintercept = 17.8, linetype = "dashed") +
      scale_x_continuous(limits = c(0, 25), breaks = seq(0, 25, 5))
    B <- ggplot(filter(df_figure2co, Survey == get(finfish_panels[2])), aes(x = `Shell Length (cm)`, fill = Lipped)) +
      geom_histogram(color = "black", position = "identity", alpha = 0.5) +
      theme_custom +
      labs(title = "B", subtitle = get(finfish_panels[2])) +
      scale_fill_manual(values = c(palette[3], palette[4])) +
      geom_vline(xintercept = 17.8, linetype = "dashed") +
      scale_x_continuous(limits = c(0, 25), breaks = seq(0, 25, 5))
    if (length(finfish_panels) == 2) {
      ...
      C <- ggplot(filter(df_figure2co, Survey == get(finfish_panels[3])), aes(x = `Shell Length (cm)`, fill = Lipped)) +
        geom_histogram(color = "black", position = "identity", alpha = 0.5) +
        theme_custom +
        labs(title = "C", subtitle = get(finfish_panels[3])) +
        scale_fill_manual(values = c(palette[3], palette[4])) +
        geom_vline(xintercept = 17.8, linetype = "dashed") +
        scale_x_continuous(limits = c(0, 25), breaks = seq(0, 25, 5))
      if (length(finfish_panels) == 3) {
        ...
        D <- ggplot(filter(df_figure2co, Survey == get(finfish_panels[4])), aes(x = `Shell Length (cm)`, fill = Lipped)) +
          geom_histogram(color = "black", position = "identity", alpha = 0.5) +
          theme_custom +
          labs(title = "D", subtitle = get(finfish_panels[4])) +
          scale_fill_manual(values = c(palette[3], palette[4])) +
          geom_vline(xintercept = 17.8, linetype = "dashed") +
          scale_x_continuous(limits = c(0, 25), breaks = seq(0, 25, 5))
