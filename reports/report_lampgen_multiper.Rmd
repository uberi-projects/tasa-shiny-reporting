...
df_table1_sum_lobster <- if (study_lobster_check) {
  df_lobster %>%
    ...
    summarise(
      ...
      `Spotted Spiny` = sum(Species == "Pg" | Species == "Panulirus guttatus" | Species == "Spotted Spiny Lobster", na.rm = TRUE)
    ) %>%
    bind_rows(
      df_lobster %>%
        summarise(
...
df_table2_urchin_crab <- if (study_urchin_crab_check) {
  site_survey_summary <- df_urchin_crab %>%
    group_by(`Site ID`, Survey) %>%
    summarise(
      Diadema_Count =
  ...
  survey_summary <- site_survey_summary %>%
    group_by(Survey) %>%
    summarise(
  ...
  overall_summary <- site_survey_summary %>%
    summarise(
      Survey = "Overall",
      `Diadema / Site` = round(mean(Diadema_Count, na.rm = TRUE), 1),
      `Maguimithrax / Site` = round(mean(Maguimithrax_Count, na.rm = TRUE), 1)
    )
  bind_rows(survey_summary, overall_summary)
} else {
  tibble(Survey = character())
}
...
overall_crab_av <- if (study_urchin_crab_check) {
  filter(df_table2, Survey == "Overall")$`Maguimithrax / Site`
}
...
if (study_conch_check) {
  ...
  conch_density_overall_queen <- filter(df_table1co, Survey == "Overall")$`Queen Conch / Ha`
  conch_density_overall_milk <- filter(df_table1co, Survey == "Overall")$`Milk Conch / Ha`
  conch_max_density <- max(filter(df_table1co, Survey != "Overall")$`Queen Conch / Ha`)
  survey_conch_max_density <- filter(df_table1co, Survey != "Overall")$Survey[which.max(df_table1co$`Queen Conch / Ha`)]
}
```
...
if (study_conch_check) {
  ...
  df_figure1co <- df_table1co_surveys %>%
    select(Survey, `Queen Conch` = `Queen Conch / Ha`, `Milk Conch` = `Milk Conch / Ha`) %>%
...
if (study_lobster_check) {
  ...
  df_table1l_overall <- df_lobster %>%
    ...
    summarise(
      ...
      `Ha Sampled` = n_distinct(paste0(`Site ID`, Transect, Survey), na.rm = TRUE) * 200 / 10000,
  ...
  df_table1l_surveys <- df_lobster %>%
    ...
    summarise(
      Sites = n_distinct(paste(Survey, `Site ID`), na.rm = TRUE),
      `Ha Sampled` = n_distinct(paste0(Survey, `Site ID`, Transect), na.rm = TRUE) * 200 / 10000,
      `Caribbean Spiny` = sum(Species == "Pa" | Species == "Panulirus argus" | Species == "Caribbean Spiny Lobster"),
      `Caribbean Spiny / Ha` = round(`Caribbean Spiny` / `Ha Sampled`, 1),
      `Spotted Spiny` = sum(Species == "Pg" | Species == "Panulirus guttatus" | Species == "Spotted Spiny Lobster"),
      Males = sum(Sex %in% c("M", "m", "Male", "male")),
  ...
  max_density_lobster_car <- max(df_table1l_surveys$`Caribbean Spiny / Ha`)
  survey_max_density_lobster_car <- df_table1l_surveys$Survey[which.max(df_table1l_surveys$`Caribbean Spiny / Ha`)]
...
if (study_lobster_check) {
  ...
  B <- ggplot(df_table1l_surveys, aes(x = Survey, y = `Caribbean Spiny / Ha`, fill = Survey)) +
    geom_col(color = "black") +
    theme_custom +
    xlab("Survey") +
    labs(title = "B") +
...
if (study_urchin_crab_check) {
  ...
  df_table1dm_overall <- df_urchin_crab %>%
    summarize(
      ...
      `Maguimithrax` = sum(`Maguimithrax spinosissumus`)
    ) %>%
    mutate(Survey = "Overall")
...
table1dmcaption <- if (study_urchin_crab_check) {
  paste(
    "**Table 1DM.** _Diadema antillarum_ and _Maguimithrax spinosissumus_ counts in surveys and overall in",
    study_year_month,
    "at Turneffe Atoll Marine Reserve.",
    "\n\n"
  )
