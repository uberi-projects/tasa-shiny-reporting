...
**Table 1.** Summary of fishing activity by month, including total fishers, events, and lobster caught, 
...
total_carapace <- filter(df_table3, Month == "Overall")$`Mean Carapace Length (In)`
total_weight <- filter(df_table3, Month == "Overall")$`Mean Weight (oz)`
max_carapace <- max(filter(df_table3, Month != "Overall")$`Mean Carapace Length (In)`, na.rm = TRUE)
max_carapace_month <- df_table3$Month[which.max(filter(df_table3, Month != "Overall")$`Mean Carapace Length (In)`)]
...
Average headed weight was `r total_weight`oz, greatest in `r max_weight_month` with `r max_weight`oz. 
