...
output$figures_lamp_multiper <- downloadHandler(
    filename = function() {
        datatype <- isolate(input$datatype_lamp_multiper)
        paste0("figure_lamp_multiper_", tolower(datatype), ".zip")
    },
    content = function(zipfile) {
        ...
        p1 <- switch(datatype,
            "Conch" = ggplot(iris, aes(Sepal.Width, Sepal.Length, color = Species)) +
                geom_point() +
                theme_classic(),
            "General LAMP" = ggplot(iris, aes(Sepal.Width, Sepal.Length, color = Species)) +
                geom_point() +
                theme_dark(),
        ...
        plot_list[[1]] <- list(
            plot = p1,
            name = paste0("figure_lamp_multiper_", tolower(datatype), "_1.png")
        )
        if (datatype == "Conch") {
            ...
            plot_list[[2]] <- list(
                plot = p2,
                name = paste0("figure_lamp_multiper_", tolower(datatype), "_2.png")
            )
        }
