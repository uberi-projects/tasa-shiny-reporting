## figures_spag_multiper.r

# Download figures ---------------------------
output$figures_spag_multiper <- downloadHandler(
    filename = function() {
        datatype <- isolate(input$datatype_spag_multiper)
        paste0("figure_spag_multiper_", tolower(datatype), ".zip")
    },
    content = function(zipfile) {
        ...
        plot_list[[1]] <- list(
            plot = p1,
            name = paste0("figure_spag_multiper_", tolower(datatype), "_1.png")
        )
        if (datatype == "Visual Census") {
            ...
            plot_list[[2]] <- list(
                plot = p2,
                name = paste0("figure_spag_multiper_", tolower(datatype), "_2.png")
            )
        }
